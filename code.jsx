...
const INITIAL_LEADS = [
  { id: 1, name: "PT Maju Jaya", contact: "Budi Santoso", source: "GMaps", status: "Hot", score: 92, email: "budi@majujaya.com", phone: "0812-3456-7890", category: "F&B", notes: "Interested in digital menu", lastContact: "2025-04-20", value: 15000000 },
  { id: 2, name: "Toko Elektronik Sinar", contact: "Dewi Rahayu", source: "DM IG", status: "Warm", score: 74, email: "dewi@sinar.co.id", phone: "0813-9876-5432", category: "Retail", notes: "Follow up minggu depan", lastContact: "2025-04-18", value: 8500000 },
  { id: 3, name: "Klinik Sehat Prima", contact: "Dr. Andi", source: "Cold Email", status: "Cold", score: 45, email: "andi@kliniksehat.com", phone: "0877-1234-5678", category: "Health", notes: "Belum respon email ke-2", lastContact: "2025-04-10", value: 22000000 },
  { id: 4, name: "Rumah Makan Padang Sederhana", contact: "Pak Rusdi", source: "GMaps", status: "Closed", score: 100, email: "rusdi@sederhana.id", phone: "0811-2233-4455", category: "F&B", notes: "Deal signed!", lastContact: "2025-04-22", value: 12000000 },
];

const INITIAL_OUTREACH = [
  { id: 1, leadName: "PT Maju Jaya", type: "Email", date: "2025-04-20", subject: "Solusi Digital untuk Bisnis Anda", status: "Replied", opens: 3, clicks: 2 },
  { id: 2, leadName: "Toko Elektronik Sinar", type: "DM Instagram", date: "2025-04-18", subject: "Halo kak, ada penawaran spesial!", status: "Seen", opens: 1, clicks: 0 },
  { id: 3, leadName: "Klinik Sehat Prima", type: "Email", date: "2025-04-10", subject: "Tingkatkan Pasien dengan Strategi Digital", status: "No Response", opens: 0, clicks: 0 },
  { id: 4, leadName: "CV Berkah Motor", type: "WhatsApp", date: "2025-04-19", subject: "Perkenalan layanan kami", status: "Rejected", opens: 1, clicks: 0 },
];

const INITIAL_REJECTIONS = [
  { id: 1, leadName: "CV Berkah Motor", date: "2025-04-19", reason: "Tidak butuh sekarang", channel: "WhatsApp", followUpDate: "2025-07-19", lesson: "Timing salah, coba 3 bulan lagi saat mereka renewal" },
  { id: 2, leadName: "Salon Cantik Abadi", date: "2025-04-15", reason: "Budget tidak ada", channel: "Email", followUpDate: "2025-06-15", lesson: "Tawarkan paket starter yang lebih affordable" },
  { id: 3, leadName: "Apotek Sehat Selalu", date: "2025-04-12", reason: "Sudah pakai kompetitor", channel: "DM IG", followUpDate: "2025-10-12", lesson: "Highlight differentiator kita vs kompetitor saat follow up" },
];

const AI_PLAYBOOK = [
  {
    category: "🗺️ Scraping Google Maps",
    color: "#00ff88",
    steps: [
  ...
  {
    category: "📩 Cold Outreach di Era AI",
    color: "#ff6b35",
    steps: [
  ...
  {
    category: "📊 Meningkatkan Sales Metrics",
    color: "#a78bfa",
    steps: [
  ...
  {
    category: "🤖 Stack AI Sales Modern",
    color: "#f59e0b",
    steps: [
...
const statusColor = { Hot: "#ff4444", Warm: "#ff9900", Cold: "#4488ff", Closed: "#00cc66" };
const statusBg = { Hot: "#ff44441a", Warm: "#ff99001a", Cold: "#4488ff1a", Closed: "#00cc661a" };

export default function SalesTracker() {
  const [activeTab, setActiveTab] = useState("Dashboard");
  const [leads, setLeads] = useState(INITIAL_LEADS);
  const [outreach, setOutreach] = useState(INITIAL_OUTREACH);
  ...
  const conversionRate = ((closedLeads.length / leads.length) * 100).toFixed(0);
  const avgScore = (leads.reduce((a, b) => a + b.score, 0) / leads.length).toFixed(0);
  const repliedOutreach = outreach.filter(o => o.status === "Replied").length;
  const replyRate = ((repliedOutreach / outreach.length) * 100).toFixed(0);
  ...
  function addLead() {
    ...
    setLeads([...leads, { ...newLead, id: Date.now(), score, lastContact: new Date().toISOString().split("T")[0], value: parseInt(newLead.value) || 5000000 }]);
  ...
  function addOutreach() {
    ...
    setOutreach([...outreach, { ...newOutreach, id: Date.now(), date: new Date().toISOString().split("T")[0], opens: 0, clicks: 0 }]);
  ...
  function addRejection() {
    ...
    setRejections([...rejections, { ...newRejection, id: Date.now(), date: new Date().toISOString().split("T")[0] }]);
    ...
    <div style={{ background: "#010409", minHeight: "100vh", fontFamily: "'IBM Plex Mono', monospace", color: "#e6edf3" }}>
      ...
      <div style={{ borderBottom: "1px solid #21262d", padding: "16px 24px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <div>
            <div style={{ fontSize: 16, fontWeight: 700, fontFamily: "'Space Grotesk', sans-serif", letterSpacing: "-0.5px" }}>SALES<span style={{ color: "#00ff88" }}>RADAR</span></div>
        ...
        <div style={{ fontSize: 11, color: "#7d8590", textAlign: "right" }}>
          <span style={{ color: "#00ff88" }}>● </span>LIVE · {new Date().toLocaleDateString("id-ID", { day: "2-digit", month: "short", year: "numeric" })}
        </div>
      </div>

      {/* Tabs */}
      <div style={{ borderBottom: "1px solid #21262d", padding: "0 24px", display: "flex", gap: 4 }}>
        {TABS.map(tab => (
          <button key={tab} className="tab-btn" onClick={() => setActiveTab(tab)}
            style={{ background: activeTab === tab ? "#161b22" : "transparent", border: "none", borderBottom: activeTab === tab ? "2px solid #00ff88" : "2px solid transparent", color: activeTab === tab ? "#e6edf3" : "#7d8590", padding: "12px 16px", fontSize: 12, cursor: "pointer", fontFamily: "'IBM Plex Mono', monospace", fontWeight: activeTab === tab ? 700 : 400, letterSpacing: "0.5px" }}>
            {tab.toUpperCase()}
          </button>
        ))}
      </div>
          ...
          <div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: 16, marginBottom: 32 }}>
              {[
                ...
                <div key={s.label} className="stat-card" style={{ background: "#0d1117", border: "1px solid #21262d", borderRadius: 12, padding: "20px 16px" }}>
        ...
        {activeTab === "Outreach" && (
          <div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              {outreach.map(o => {
                const statusColors = { Replied: "#00ff88", Seen: "#f59e0b", Sent: "#00ccff", "No Response": "#7d8590", Rejected: "#ff4444" };
                return (
                ...
                <div key={si} style={{ background: "#0d1117", border: "1px solid #21262d", borderRadius: 14, padding: 24 }}>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 12 }}>
                    {section.steps.map((step, i) => (
                      <div key={i} className="step-card" style={{ "--accent": section.color, background: "#161b22", borderRadius: 10, padding: 16, border: "1px solid #21262d", transition: "border-color 0.2s" }}>
      ...
      {showAddLead && (
        <div className="modal-overlay" onClick={() => setShowAddLead(false)}>
          <div onClick={e => e.stopPropagation()} style={{ background: "#0d1117", border: "1px solid #21262d", borderRadius: 16, padding: 28, width: "100%", maxWidth: 500 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
              {[["name", "Nama Perusahaan"], ["contact", "Nama Kontak"], ["email", "Email"], ["phone", "No. HP"], ["value", "Estimasi Value (Rp)"]].map(([k, label]) => (
                <div key={k} style={{ gridColumn: k === "name" || k === "value" ? "1/-1" : "auto" }}>
                  ...
                  <input value={newLead[k]} onChange={e => setNewLead({ ...newLead, [k]: e.target.value })} style={inputStyle} placeholder={label} />
                </div>
              ))}
              {[["source", "Source", ["GMaps", "DM IG", "Cold Email", "Referral", "WhatsApp", "LinkedIn"]], ["status", "Status", ["Cold", "Warm", "Hot", "Closed"]], ["category", "Kategori", ["F&B", "Retail", "Health", "Property", "Service", "Tech", "Education"]]].map(([k, label, opts]) => (
                <div key={k}>
                  <label style={{ fontSize: 11, color: "#7d8590", display: "block", marginBottom: 6 }}>{label}</label>
                  <select value={newLead[k]} onChange={e => setNewLead({ ...newLead, [k]: e.target.value })} style={inputStyle}>
      ...
      {showAddOutreach && (
        <div className="modal-overlay" onClick={() => setShowAddOutreach(false)}>
          <div onClick={e => e.stopPropagation()} style={{ background: "#0d1117", border: "1px solid #21262d", borderRadius: 16, padding: 28, width: "100%", maxWidth: 440 }}>
            ...
            {[["leadName", "Nama Lead/Perusahaan"], ["subject", "Subject / Pesan Pembuka"]].map(([k, label]) => (
              <div key={k} style={{ marginBottom: 12 }}>
                <label style={{ fontSize: 11, color: "#7d8590", display: "block", marginBottom: 6 }}>{label}</label>
                <input value={newOutreach[k]} onChange={e => setNewOutreach({ ...newOutreach, [k]: e.target.value })} style={inputStyle} placeholder={label} />
              </div>
            ))}
            {[["type", "Tipe", ["Email", "DM Instagram", "WhatsApp", "LinkedIn", "Telepon"]], ["status", "Status", ["Sent", "Seen", "Replied", "No Response", "Rejected"]]].map(([k, label, opts]) => (
              <div key={k} style={{ marginBottom: 12 }}>
                <label style={{ fontSize: 11, color: "#7d8590", display: "block", marginBottom: 6 }}>{label}</label>
                <select value={newOutreach[k]} onChange={e => setNewOutreach({ ...newOutreach, [k]: e.target.value })} style={inputStyle}>
                  {opts.map(o => <option key={o} value={o}>{o}</option>)}
                </select>
              </div>
            ))}
      ...
      {showAddRejection && (
        <div className="modal-overlay" onClick={() => setShowAddRejection(false)}>
          <div onClick={e => e.stopPropagation()} style={{ background: "#0d1117", border: "1px solid #21262d", borderRadius: 16, padding: 28, width: "100%", maxWidth: 440 }}>
            ...
            {[["leadName", "Nama Lead/Perusahaan"], ["reason", "Alasan Rejection"], ["lesson", "Lesson Learned (apa yang lo pelajari?)"]].map(([k, label]) => (
              <div key={k} style={{ marginBottom: 12 }}>
                <label style={{ fontSize: 11, color: "#7d8590", display: "block", marginBottom: 6 }}>{label}</label>
                {k === "lesson" ? <textarea value={newRejection[k]} onChange={e => setNewRejection({ ...newRejection, [k]: e.target.value })} style={{ ...inputStyle, resize: "none", height: 70 }} placeholder={label} /> : <input value={newRejection[k]} onChange={e => setNewRejection({ ...newRejection, [k]: e.target.value })} style={inputStyle} placeholder={label} />}
              </div>
            ))}
          ...
          <div onClick={e => e.stopPropagation()} style={{ background: "#0d1117", border: "1px solid #21262d", borderRadius: 16, padding: 28, width: "100%", maxWidth: 480 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
              <div>
                ...
                <span className="badge" style={{ background: statusBg[selectedLead.status], color: statusColor[selectedLead.status], border: `1px solid ${statusColor[selectedLead.status]}30`, marginTop: 6 }}>{selectedLead.status}</span>
              </div>
            ...
            {[["👤 Kontak", selectedLead.contact], ["📧 Email", selectedLead.email], ["📱 Phone", selectedLead.phone], ["🏷️ Kategori", selectedLead.category], ["📍 Source", selectedLead.source], ["📅 Last Contact", selectedLead.lastContact]].map(([k, v]) => (
